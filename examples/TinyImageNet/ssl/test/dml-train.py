...
parser.add_argument("--gates", default=["ThroughGate"])
parser.add_argument(
    "--ssls",
...
def objective(trial):
    ...
    nodes = []
    ssl_name = trial.suggest_categorical("ssl", ssls_name)
    for i in range(num_nodes):
        ...
        writer = SummaryWriter(
            f"runs/dml_{num_nodes}/{ssl_name}/{transforms_name}/{trial.number:04}/{i}_{model_name}_{ssl_name}"
        )
        save_dir = f"checkpoint/dml_{num_nodes}/{ssl_name}/{transforms_name}/{trial.number:04}/{i}_{model_name}_{ssl_name}"
